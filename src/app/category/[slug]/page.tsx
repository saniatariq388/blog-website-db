...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
   <>
      <div className="min-h-screen flex flex-col bg-primary-50 justify-center mt-20">
      
      <main className="flex-grow container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row gap-8">
          <div className="w-full md:w-3/4">
            <h1 className="text-2xl md:text-3xl font-bold mb-6 text-secondary-700 capitalize">{slug} Posts</h1>
            <div className="w-3/4 grid grid-rows-1 md:grid-rows-2 gap-8 justify-center items-center ml-[80px] mt-20">
              {categoryPosts.map((post) => (
                <div key={post.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                  <Image src={post.image} alt={post.title} width={600} height={400} className="w-full h-58 object-cover" />
                  <div className="p-4">
